...
## Motiro Country report template: prepares the text, plots and graphs for a NS country level report
## A country report is the result of a nation-wide Motiro process, where several teams, regions, cantons, branches, etc.
# have conducted the Motiro survey and team discussions.
# When team discussions are completed, the country report is prepared to summarize and triangulate
...
# variables for the number of teams and regions surveyed
number_teams_surveyed = df['Team Name'].nunique()
number_regions_surveyed = df['region'].nunique()
...
doc.add_picture(f'{structure_name}_spider.png', width=Inches(6), height=Inches(6))
...
doc.add_picture(f'{NS}_{country_name}_wellbeing_bar.png', width=Inches(6), height=Inches(2.5))
...
doc.add_picture(f'{NS}_{country_name}_engagement_spider.png', width=Inches(3), height=Inches(3))

doc.add_paragraph(f'<Paste the regional bar graph here>')

# Load the CSV file into a DataFrame
engagement_df = pd.read_csv(f'{NS}_{country_name}_Engagement.csv')
...
doc.add_picture(f'{NS}_{country_name}_needs_spider.png', width=Inches(2), height=Inches(2))
...
doc.add_picture(f'{NS} VOL SDTCorrNetworkGraph.png', width=Inches(6), height=Inches(4))
